...
@behave.when('clico em "Cadastre-se para fazer entregas" sem preencher nenhum campo')
def step_clico_cadastrar_sem_preencher(context: behave.runner.Context) -> None:
    locator_tuple = (By.CLASS_NAME, "button-success")
    context.page.click(locator_tuple)

@behave.when('preencho o CPF com "{cpf_invalido}"')
def step_preencho_cpf_alfanumerico(context: behave.runner.Context, cpf_invalido: str) -> None:
...
@behave.then('quando clicar em "OK" na mensagem de sucesso, devo ser redirecionado para a pagina inicial')
...
def step_ver_mensagem_cpf_invalido(context: behave.runner.Context) -> None:
    context.page.wait_element_present((By.CLASS_NAME, "alert-error"), timeout=20)
    context.page.get_text(By.XPATH, "//*[@id='page-deliver']/form/fieldset[1]/div[1]/div[2]/span", "Oops! CPF inválido")
